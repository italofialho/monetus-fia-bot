...
const businessDaysAndHours = (ctx, next) => {
  ...
  if (isHoliday) {
    return ctx.replyWithMarkdown(
      `🤖 Hoje muito provavelmente é feriado ${utils.getFirstNameOrUserName(ctx.message.from)}, e o mercado não está operando agora! Por isso não é possível visualizar o desempenho do FIA 😖`
    );
  }
