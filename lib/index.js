const { Composer, Extra, log, session, filter } = require('micro-bot')
...
const dropOldUpdates = filter(({ message }) => {
  const now = new Date().getTime() / 1000
  return !message || message.date > (now - 30)
})
...
bot.init = (ctx) => {
  return new Promise((resolve) => {
    ...
    if (process.env.ENV !== 'dev') {
      let message = `🤖 Oi *@italofialho*, estou no ar! Agora são *${moment().format(
        'HH:mm:ss'
      )}*.`

      if (!isHoliday && !isBusinessDay) {
        if (isHoliday) {
          message += ` No final de semana o mercado não está aberto por isso eu vou desativar as notificações diarias e de hora em hora!`
        }
      }

      if (isHoliday) {
        message += ` Hoje é feriado (${utils.currentHoliday(
          now.format('DD/MM')
        )}) por isso o mercado não está aberto e eu vou desativar as notificações diarias e de hora em hora!`
      }

      ctx.telegram.sendMessage(process.env.BOT_ADMIN_ID, message, {
        parse_mode: 'markdown'
      })
    }
    ...
    mongoose
      ...
      .then(() => {
        console.log('μ-bot: Connected to mongo')
        // Prepare DB
        setupDB()
        // Schedule crons
...
bot.command('produtos', ({ replyWithMarkdown }) => replyWithMarkdown(commands.products()))
