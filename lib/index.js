...
mongoose
  ...
  .then(() => {
    ...
    bot.init = (ctx) => {
      return new Promise((resolve) => {
        ...
        ctx.telegram.sendMessage(
          process.env.BOT_ADMIN_ID,
          `🤖 Oi *Italo*, estou no ar! Agora são *${moment().format(
            'HH:mm:ss'
          )}* no meu horário local e são *${momentTZ()
            .tz('America/Sao_Paulo')
            .format('HH:mm:ss')}* no horário de Brasilia.`,
          {
            parse_mode: 'markdown'
          }
        )
    ...
    bot.command(
      'c',
      ({ replyWithMarkdown, replyWithPhoto, message, telegram }) =>
        commands
          .composition(message)
          .then(({ image, error }) => {
            if (error) {
    ...
    bot.command(
      'q',
      ({ replyWithPhoto, replyWithMarkdown, message, telegram }) =>
        commands
          .quote(message)
    ...
    bot.command('force_update', ({ replyWithMarkdown, message, telegram }) => {
      adminCommands
        .forceUpdate(message)
        .then(({ isAdmin }) => {
          if (isAdmin) { replyWithMarkdown('✅ Informações atualizadas com sucesso!') } else {
            replyErrorToAdmin(
              telegram,
              message,
