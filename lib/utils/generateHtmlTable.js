...
const generateHtmlTable = ({ assets, delta, benchmark }, options = { delta: true }) => {
  try {
    const header = `
    <thead>
      <tr>
  ...
  `
    ...
    <tfoot>
      ...
      <tr>
        <td class='bold'>IBOV</td>
        <td></td>
        <td></td>
        ${(options.delta ? `<td class="right bold ${deltaClass(benchmark.delta)}">${formatDelta(benchmark.delta)}</td>` : '')}
      </tr>
      <tr>
        <td class='bold'>ALFA</td>
        <td>FIA - IBOV</td>
        <td></td>
        ${(options.delta ? `<td class="right bold ${deltaClass(delta - benchmark.delta)}">${formatDelta(delta - benchmark.delta)}</td>` : '')}
      </tr>
    </tfoot>
  `
