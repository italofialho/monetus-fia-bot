...
const generateQuoteDetailsHtmlTable = (quote, asset) => {
  ...
  const header = `
    <main class="row title">
        <ul>
          <li style="width: 100%; text-align: center;">
            Detalhes - ${asset.name} (${asset.ticker})
          </li>
        </ul>
      </main>
  `
...
`
  ...
  const disclaimer = `
    <main class="row title">
      <ul>
        <li style="width: 100%; text-align: center;" class="font-small-2">
          Os detalhes do ativo <b>${asset.ticker}</b> são atualizados seguindo as informações página do <b>Comparador de Fundos</b> da própria Monetus.
        </li>
      </ul>
    </main>
`
