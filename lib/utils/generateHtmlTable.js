...
`
  ...
  assets.forEach(asset => {
    console.log(`DEBUG: ${asset.ticker} - ${asset.quote.delta} - ${deltaClass(asset.quote.delta)} - ${formatDelta(asset.quote.delta)} - ${asset.allocation} - ${asset.allocation / 100}`)
    content += `
      <section class="row-fadeOut-wrapper">
        <article class="row fadeIn ${deltaClass(asset.quote.delta)}">
          <ul>
            <li><a href="#" class="bold">${asset.ticker}</a></li>
            ...
            <li class="right">${asset.name}</li>
            <li>${moment(asset.quote.requestTimestamp).tz('America/Sao_Paulo').format('DD/MM HH:mm')}</li>
          </ul>
        </article>
      </section>
    `
