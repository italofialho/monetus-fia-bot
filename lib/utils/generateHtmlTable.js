...
const generatePerformaceHtmlTable = ({ assets, delta, benchmark }, options = { delta: true }) => {
  ...
  const lastUpdatedDate = moment(assets[0].quote.requestTimestamp).format('DD [de] MMMM [de] YYYY [às] HH:mm')
    ...
    <main class="row title">
      <ul>
        <li style="width: 100%; text-align: center;" class="font-small-2">
          A composição do <b>Monetus FIA</b> é atualizado seguindo as informações página do <b>Comparador de Fundos</b> da própria Monetus.
        </li>
      </ul>
    </main>
`
...
const generateCompositionHtmlTable = (composition, options = { delta: true }) => {
  const lastUpdatedDate = moment(composition[0].created_at).format('DD [de] MMMM [de] YYYY [às] HH:mm')
  const header = `
    <main class="row title">
        <ul>
          <li style="width: 100%; text-align: center;">
...
const generateTickerDetailsHtmlTable = (composition) => {
  const lastUpdatedDate = moment(composition.created_at).format('DD [de] MMMM [de] YYYY [às] HH:mm')
  const header = `
    <main class="row title">
        <ul>
          <li style="width: 100%; text-align: center;">
            Detalhes - ${composition.name} (${composition.ticker})
          </li>
        </ul>
      </main>
  `
  ...
  let content = `
    <section class="row-fadeOut-wrapper">
      <article class="row fadeIn zero">
        <ul>
          <li><a href="#" class="zero bold">${composition.ticker}</a></li>
...
`
  ...
  const disclaimer = `
    <main class="row title">
      <ul>
        <li style="width: 100%; text-align: center;" class="font-small-2">
          Os detalhes do(a) <b>${composition.ticker}</b> são atualizados seguindo as informações página do <b>Comparador de Fundos</b> da própria Monetus.
        </li>
      </ul>
    </main>
`
...
module.exports = { generatePerformaceHtmlTable, generateCompositionHtmlTable, generateTickerDetailsHtmlTable }
