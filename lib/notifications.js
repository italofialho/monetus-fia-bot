...
function broadcastNotifications(telegram, messageBuilder, frequency = "daily") {
  ...
  if (isHoliday || !isBusinessDay) {
    console.log(
      "μ-bot: Today is holiday or not is business day! Disabling automatic notifications."
    );
    if (frequency === "daily") {
    ...
    .then((performance) => {
      getPerformanceImage()
        .then((image) => {
          ...
          getAllSubscribed(frequency)
            .then((chatIds) => {
              chunk(chatIds, 30).forEach((ids, i) => {
                // Telegram limits to 30 msg/s
                ((i, ids) => {
                  // Wait 1.5 seconds between each chunk
                  setTimeout(() => {
                    // This makes me hate JS so much
                    ids.forEach((id) => {
                      telegram.sendPhoto(
                        id,
                        { source: image },
