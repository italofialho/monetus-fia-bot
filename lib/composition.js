...
function fetchComposition () {
  return new Promise((resolve, reject) => {
    axios.get(
      'https://proxy.monetus.com.br/api/fund/monetus-fia/composition',
      {
        params: {
          isPrivatePension: false
        },
        headers: {
          'X-Requested-With': 'XMLHttpRequest',
          'Cookie': `__t=${process.env.MONETUS_COOKIE_TOKEN}`
        }
      }
    ).then(response => {
      if (response.status === 200) {
        const data = response.data.map(asset => {
          ...
          return {
            ...
            stockId: parseInt(asset.id),
            ticker: ticker,
            name: name,
            description: String(asset.description).trim(),
            allocation: parseFloat(asset.percentage),
            createdAt: new Date(),
            updatedAt: new Date(),
